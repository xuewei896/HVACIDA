    ...
    public class SmallSystemLoadCalculator : ISmallSystemLoadCalculator
    {
        ...
        private static SmallSystemResult CalculateAllAirOnceReturn(SmallSystemInput x)
        {
            var r = new SmallSystemResult
            {
                ...
                PendingNote = "口径说明:① 消除余热通风量公式中的「送风点焓」在公式文档里未单独定义," +
                              "按文档示例反算即为送风点焓(示例:9.92/(54.30−43.90)/1.15×3600 ≈ 2985 ≈ 表列 2985),本实现按送风点焓取值;" +
                              "② 文档示例工程照明指标用 20 W/m²、换气次数取默认值,与文档所述默认 8 W/m² 不同 —— 两者均为可改输入。"
            };
        ...
        private static SmallSystemResult CalculateVrfWithFreshAir(SmallSystemInput x)
        {
            var r = new SmallSystemResult
            {
                ...
                PendingNote = "口径说明:① 消除余热通风量的分母按公式文档为「1.01 × (室内温度 − 过渡季通风室外温度)」," +
                              "即按过渡季温差计算;② 文档示例的室内状态点焓 61.80 对应约 29 ℃ / 50%," +
                              "与文档所述默认室内 27 ℃ 不一致 —— 室内温度为可改输入,自检按文档示例取值复算。"
            };
        ...
        private static SmallSystemResult CalculateExhaust(SmallSystemInput x)
        {
            var r = new SmallSystemResult
            {
                ...
                PendingNote = "口径存疑:公式文档文字写「选型排风量为计算排风量×1.1」,但同一个系统的示例(EAF-A601)" +
                              "用的是 **1.3**(5386×1.3 = 7002)。代码默认取文档文字的 1.1 并允许修改;" +
                              "自检对示例口径(1.3 → 7002)单独断言。请确认以哪个为准。"
            };
        ...
        private static SmallSystemResult CalculatePressurization(SmallSystemInput x)
        {
            var r = new SmallSystemResult
            {
                ...
                Note = "门面积 = 一层内可开启门宽度 × 门高度;" +
                       ...
                       "选型风量 = 楼梯间加压送风量 × 1.2。",
                PendingNote = "口径说明:公式文档把门开启风量、门缝漏风量、余压阀漏风量的单位写作 m³/s," +
                              "加压送风量与选型风量也写作 m³/s,但加压送风量明显是乘 3600 之后的结果(文档单位标注不一致)。" +
                              "本实现统一按 m³/h 输出、选型系数 1.2 不变;若你那边口径是 m³/s,请告知我改。"
            };
